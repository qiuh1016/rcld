...
// activity
let HRToastActivityWidth  :  CGFloat  = 96.0
let HRToastActivityHeight :  CGFloat  = 96.0
...
extension UIView {
    ...
    */
    ...
    // okView
    func makeOKToast(message msg: String, ok: Bool) {
        UIView.hr_setToastThemeColor(color: UIColor(white: 0.3, alpha: 0.8))
        
        let existingActivityView: UIView? = objc_getAssociatedObject(self, &HRToastActivityView) as? UIView
        if existingActivityView != nil { return }
        
        let okView = UIView(frame: CGRectMake(0, 0, HRToastActivityWidth, HRToastActivityHeight))
        okView.center = self.centerPointForPosition(HRToastActivityPositionDefault, toast: okView)
        okView.backgroundColor = UIView.hr_toastThemeColor().colorWithAlphaComponent(HRToastOpacity)
        okView.alpha = 0.0
        okView.autoresizingMask = ([.FlexibleLeftMargin, .FlexibleTopMargin, .FlexibleRightMargin, .FlexibleBottomMargin])
        okView.layer.cornerRadius = HRToastCornerRadius
        
        if HRToastDisplayShadow {
            okView.layer.shadowColor = UIView.hr_toastThemeColor().CGColor
            okView.layer.shadowOpacity = Float(HRToastShadowOpacity)
            okView.layer.shadowRadius = HRToastShadowRadius
            okView.layer.shadowOffset = HRToastShadowOffset
        }
        
        let imageView = ok ? UIImageView(image: UIImage(named: "Checkmark")) : UIImageView(image: UIImage(named: "error"))
        imageView.center = CGPointMake(okView.bounds.size.width / 2, okView.bounds.size.height / 2)
        okView.addSubview(imageView)
        
        imageView.frame.origin.y -= 10
        let okLabel = UILabel(frame: CGRectMake(okView.bounds.origin.x, (imageView.frame.origin.y + imageView.frame.size.height + 0), okView.bounds.size.width, 20))
        okLabel.textColor = UIView.hr_toastFontColor()
        okLabel.font = (msg.characters.count<=10) ? UIFont(name:UIView.hr_toastFontName(), size: 16) : UIFont(name:UIView.hr_toastFontName(), size: 13)
        ...
        objc_setAssociatedObject(self, &HRToastActivityView, okView, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
